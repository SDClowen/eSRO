...
int ConnectionState::Parse (IPacket &packet)
{
    ...
    {
        syslog(LOG_INFO,"clt::ConnectionState::Parse - Unknown opcode %X",packet.opcode());
    ...
    {
        syslog(LOG_INFO,"clt::IConnectionState::Parse - Exception: %s",e.what());
        ret = MSG_SUCCESS;
    }
    catch (...)
    {
        syslog(LOG_INFO,"clt::IConnectionState::Parse - Unhandled error.");
        ret = MSG_ERROR;
    }
